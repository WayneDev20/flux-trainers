#!/usr/bin/env bash
# ============================================================
# FLUX LoRA Trainer — Vast.ai end-to-end automation
#
# Usage:
...
#   --trigger_word <word>       LoRA trigger word (default: TOK)
...
#                               (re-use with --instance_id on next run)
#   --instance_id <id>          Re-use a running instance (skip rent+setup)
#   --skip_setup                Skip setup.sh (weights already downloaded)
#
# Examples:
#   # Full run — rent, setup, train, download, destroy
#   ./vastai_train.sh --images ./my_images.zip --trigger_word TOK --steps 1000
#
#   # Debug run (50 steps, keep instance for next run)
#   ./vastai_train.sh --images ./my_images.zip --steps 50 --keep
#
#   # Re-use existing instance (skip 24 GB download)
#   ./vastai_train.sh --images ./my_images.zip --steps 1000 \
#       --instance_id 12345678 --skip_setup
# ============================================================
set -euo pipefail
...
REMOTE_DIR="/workspace/flux-dora-trainer"
...
info ""
info "INFERENCE: load the DoRA at scale 1.5 (not 1.0)."
